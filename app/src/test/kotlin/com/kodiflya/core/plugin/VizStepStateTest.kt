...
// Validates the stateless rendering contract: each VizStep must carry enough data
...
class VizStepStateTest {

    @Test
    fun `VizStep Sort carries full array snapshot in every step`() {
        val values = intArrayOf(3, 1, 2)
        val step = VizStep.Sort(
            values = values,
            comparing = setOf(0, 1),
            swapping = emptySet(),
            sorted = emptySet(),
    ...
    @Test
    fun `VizStep Grid carries full 9x9 cell map for stateless rendering`() {
        val gridSize = 9
        ...
        for (row in 0 until gridSize) {
            for (col in 0 until gridSize) {
                assertNotNull(step.cells[Pair(row, col)], "Cell ($row,$col) missing from VizStep.Grid")
            }
        }
    }

    @Test
    fun `VizStep Tree carries full node state map for stateless rendering`() {
        ...
        for (nodeId in 1..7) {
            assertNotNull(step.nodeStates[nodeId], "Node $nodeId missing from VizStep.Tree")
        }
    ...
    fun `simulated BFS produces steps with full cell maps`() {
        ...
        val steps = mutableListOf<VizStep.Grid>()
    ...
    fun `simulated Inorder BST produces steps with full traversal sequence`() {
        ...
        val steps = mutableListOf<VizStep.Tree>()
